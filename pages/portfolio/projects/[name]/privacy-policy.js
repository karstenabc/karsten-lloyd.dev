...
function PrivacyPolicy({ project, policy, sections }) {
  return (
      <>
        <Layout page='portfolio' header={<><h1>PRIVACY POLICY</h1><p className={styles.subheading}>Last Updated: {formatDate(policy.date)}</p></>}>
          <Head>
          ...
          <Container>
            <nav aria-label="breadcrumb">
              <ol className={`breadcrumb ${styles.breadCrumb}`}>
                ...
                <li className="breadcrumb-item" aria-current="page"><a href={"/portfolio/projects/" + project.name}>{project.title}</a></li>
            ...
            <ul>
              {sections && sections.map((section, index) => (
                <li key={index}><a href={"#section" + section.section}>{section.title}</a></li>
              ))}
            </ul>
            <br/>
