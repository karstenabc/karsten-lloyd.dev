...
function nav(title, path, active) {
  if (active) {
    return (
      <Nav.Link href={path} className={styles.navActive}>
        {title}
      </Nav.Link>
    );
  } else {
    return (
      <Nav.Link href={path} style={{ color: 'white' }}>
        {title}
      </Nav.Link>
    );
  }
  ...
  return (
    <div>
      <Head>
        ...
        <meta name='Karsten Lloyd' content='Online portfolio' />
      ...
      {page !== 'home' && page !== 'project' && (
        <div className={styles.title + ' ' + fonts.title}>{header}</div>
      )}
      {page !== 'home' && page === 'project' && <div className={styles.jumbotron}>{header}</div>}
      <main>{children}</main>
    </div>
  );
}
